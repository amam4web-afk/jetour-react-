...
var request = {
  /**
   * get请求
   * @param url 请求的接口URL，传入如 http://www.xxx.com/xxx
...
var kefu = {
  ...
  document: {
    ...
    get: function () {
      if (kefu.document.iframeId.length < 1) {
        ...
        return document.getElementById(kefu.document.iframeId).contentWindow
          .document;
      }
    },
  ...
  notification: {
    ...
    execute: function (title, text) {
      ...
      if (document.location.protocol != "https:") {
        console.log(
          "当前使用的不是https请求！只有https请求才可以有浏览器消息通知。这里只是声音通知"
        );
        return;
      }
    ...
    audio: {
      ...
      load: function () {
        if (
          kefu.notification.audioPath == null ||
          kefu.notification.audioPath.length < 1
        ) {
          console.log(
            "已将 kefu.notification.audioPath 设为空，将不再出现声音提醒"
          );
          return;
        }
        ...
        xhr.onload = function (e) {
          //下载完成
          kefu.notification.audio.audioContext.decodeAudioData(
            this.response,
            function (buffer) {
              //解码成功时的回调函数
      ...
      play: function () {
        ...
        var audioSource =
          kefu.notification.audio.audioContext.createBufferSource();
  ...
  getMyUser: function (func) {
    ...
    request.post(
      kefu.api.get(kefu.api.getMyUser),
  ...
  ubb: function (text) {
    ...
    return text
      ...
      .replace(
        /\[li\]/g,
        '<li onclick="kefu.chat.question(this);" class="question">'
      )
      .replace(/\[\/li\]/g, "</li>")
      .replace(/\[br\]/g, "<br>");
  },
  ...
  getReceiveMessageText: function (message) {
    if (message.extend != null && message.extend.name != null) {
      ...
      if (message["text"] != null && message["text"].length < 0) {
        ...
        //将json变为插件显示的样式
        if (
          typeof kefu.extend[message.extend.name] != "undefined" &&
          typeof kefu.extend[message.extend.name].format == "function"
        ) {
  ...
  ui: {
    ...
    list: {
      renderAreaId: "kefu", //渲染区域的id，如果不赋值，那么默认就是渲染到body
      ...
      render: function () {
        if (kefu.ui.list.renderAreaId.length > 0) {
          //有设置渲染区域，那么渲染到设置的id上
          if (
            kefu.document.get().getElementById(kefu.ui.list.renderAreaId) !=
            null
          ) {
            kefu.document
              .get()
              .getElementById(kefu.ui.list.renderAreaId).innerHTML =
              kefu.ui.list.html;
          }
        } else {
        ...
        if (kefu.ui.list.listItemTemplate.length < 1) {
          //设置某个用户聊天item的模板
          kefu.ui.list.listItemTemplate = kefu.document
            .get()
            .getElementById("chatlist").innerHTML;
        }
    ...
    chat: {
      renderAreaId: "kefu", //渲染区域的id，如果不赋值，那么默认就是渲染到body
      html: `
				<div id="mobile" onclick="kefu.ui.chat.closeAll()">
					<div class="chat_header" id="head">
				        <div class="title" id="title"><img id="nickname" src="/images/chat/logo.png" /><img id="chat-close-img" src="/images/chat/button.png"/></div>
				    </div>
				    ...
				    <div id="chat_footer">
                <div class="btns">
                  <div class="box" onclick="kefu.ui.chat.sendAutoPlay()">
                    <div>Common Questions</div>
                    <img src="/images/common/O-R.png" />
                  </div>
                  <div class="box" onclick="kefu.ui.chat.openContactUs()">
                    <div>Contact Us</div>
                    <img src="/images/common/O-R.png" />
                  </div>
                </div>
                        ...
                        <textarea onkeydown="kefu.ui.chat.calAge(event)" name="" id="text" cols="30" onclick="kefu.ui.chat.textInputClick();" placeholder="Type your message here" rows="10"></textarea>
                        <!-- <div id="text" contenteditable="true" onclick="kefu.ui.chat.textInputClick();"></div> -->
                        <div class="send" id="sendButton" onclick="kefu.chat.sendButtonClick();" style="background-color:{shuruTypeColor};">Submit</div>
                        </div>
				            <div id="inputExtend">
				                <!-- 其他，如图片、商品、订单 -->
            ...
            <div id="dialog">
            <div class="chat_header" id="head1" onclick='kefu.ui.chat.cancel()'>
            <div class="title" id="title"><img id="nickname" src="/images/chat/logo.png" /><img id="chat-close-img" src="/images/chat/button.png"/></div>
        </div>
              <div class="title1">Information registration form</div>
            <div class="form">
              <div class="box1">
                <input id="first-name" type="text" placeholder="*First Name"></input>
                <div id="first-name-req" class="required-left">First Name is a required field.</div>
                <input id="last-name" type="text" placeholder="*Last Name"></input>
                <div id="last-name-req" class="required-right">Last Name is a required field.</div>
              </div>
              <div class="box2">
                <div class="left">
                  <input onclick="kefu.ui.chat.showCountryUl(event)" id="country-chat-input" oninput="kefu.ui.chat.changeCountry()" type="text" placeholder="*Country"></input>
                  <img src="/images/contact/arrow.png" alt="" />
                  <ul id="country-chat"></ul>
                  <div id="country-req" class="required-left">Country is a required field.</div>
                </div>
                <div class="left">
                  <input id="business-chat-input" readOnly onclick="kefu.ui.chat.showBusinessUl(event)" type="text" placeholder="*Business"></input>
                  <img src="/images/contact/arrow.png" alt="" />
                  <ul id="business-chat">
                    <li onclick="kefu.ui.chat.chooseBusiness('Business')">Business</li>
                    <li onclick="kefu.ui.chat.chooseBusiness('Purchase Intention')">Purchase Intention</li>
                    <li onclick="kefu.ui.chat.chooseBusiness('Test Drive')">Test Drive</li>
                    <li onclick="kefu.ui.chat.chooseBusiness('Service Support')">Service Support</li>
                  </ul>
                  <div id="business-req" class="required-right">Business is a required field.</div>
                </div>
              </div>
              <div class="box3">
                <input id="e-mail" type="email" placeholder="*E-mail"></input>
                <div id="email-req" class="required-left">E-mail is a required field.</div>
              </div>
              <div class="box3">
                <input id="mobileVal" type="text" placeholder="Mobile"></input>
              </div>
            </div>
            <div class="privacy">
              <div class="box" onclick="kefu.ui.chat.agreePrivacy()">
               <img id="agreePrivacy" src="/images/common/select.png" alt="" /> 
              </div>
              <div class="text">I have read and agree to the <span onclick="window.open('/agree')">Privacy Policy.</span></div>
              <div id="privacy-req" class="required-left">Please agree to the privacy agreement.</div>
            </div>
            <div class="description">Please confirm that the information is correct so that the 4S store can contact you</div>
            <div onclick="kefu.ui.chat.confirm();" class="submit">
              Submit
            </div>
          </div>
				</div>
			`,
      sendAutoPlay: function () {
        kefu.socket.send(
          JSON.stringify({
            token: kefu.token.get(),
      ...
      confirm: function () {
        ...
        let countryChatInput = $("#country-chat-input").val();
        let businessChatInput = $("#business-chat-input").val();
        let eMail = $("#e-mail").val();
        let mobile = $("#mobileVal").val();
        let firstNameReq = $("#first-name-req");
        let lastNameReq = $("#last-name-req");
        let countryReq = $("#country-req");
        let businessReq = $("#business-req");
        let emailReq = $("#email-req");
        let privacyReq = $("#privacy-req");
        let reg = /^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
        if (!firstName) {
          firstNameReq.css("display", "block");
          return;
        } else {
          firstNameReq.css("display", "none");
        }
        if (!lastName) {
          lastNameReq.css("display", "block");
          return;
        } else {
          lastNameReq.css("display", "none");
        }
        if (!countryChatInput) {
          countryReq.css("display", "block");
          return;
        } else {
          countryReq.css("display", "none");
        }
        if (!businessChatInput) {
          businessReq.css("display", "block");
          return;
        } else {
          businessReq.css("display", "none");
        }
        if (!reg.test(eMail)) {
          emailReq.css("display", "block");
          return;
        } else {
          emailReq.css("display", "none");
        }
        if (isSelect) {
          privacyReq.css("display", "block");
        } else {
          privacyReq.css("display", "none");
        }
        let name = countryList.find((item) => {
          return item.country_name_en == countryChatInput;
        });
        if (name) {
          black = true;
        }
        let data = {
          ...
          mail: eMail,
          tel: mobile,
          activity_name: businessChatInput,
          channel: "kefu",
          langCode: "en",
        };
        console.log(data);
          ...
          $.ajax({
            ...
            success: function (res) {
              if (res.code == 200) {
                ...
                $("#first-name").val("");
                $("#last-name").val("");
                $("#country-chat-input").val("");
                $("#business-chat-input").val("");
                $("#e-mail").val("");
                $("#mobileVal").val("");
                isSelect = false;
                black = false;
      ...
      // 打开选择国家
      showCountryUl: function (e) {
        e.stopPropagation();
        document.getElementById("country-chat").style.display = "block";
      },
      // 打开选择类型
      showBusinessUl: function (e) {
        e.stopPropagation();
        document.getElementById("business-chat").style.display = "block";
      },
      // 关闭选择国家
      closeCountryUl: function () {
        document.getElementById("country-chat").style.display = "none";
      },
      // 关闭选择类型
      closeBusinessUl: function () {
        document.getElementById("business-chat").style.display = "none";
      },
      // 关闭所有ul列表
      closeAll: function () {
        kefu.ui.chat.closeCountryUl();
        kefu.ui.chat.closeBusinessUl();
      },
      // 同意隐私协议
      agreePrivacy: function () {
        isSelect = !isSelect;
        if (isSelect) {
          $("#agreePrivacy").css("display", "none");
        } else {
          $("#agreePrivacy").css("display", "block");
        }
      },
      ...
      appendMessage: function (message, isHistory) {
        ...
        if (message.type == "QUESTION") {
          ...
          if (
            ...
            var urls = message.text.match(reg);
            var brs = [];
            var brIndex = message.text.indexOf("<br><br>");
            while (brIndex > -1) {
              brs.push(brIndex); //将出现位置赋给数组
              brIndex = message.text.indexOf("<br><br>", brIndex + 1); //从给定字符出现位置的后一位开始查找
            }
            if (urls && urls.length > 0 && brs.length > 0) {
              message.text = message.text.replaceAll("<br><br>", "<a>");
              for (var i = 0; i < urls.length; i++) {
                message.text = message.text.replace(urls[i], "</a>");
                message.text = message.text.replace(
                  "<a>",
                  "<a href='" + urls[i] + "' target='_blank'>"
                );
              }
              message.text = message.text.replace("<a href", "<br><br><a href");
              for (var i = 0; i < urls.length; i++) {
                if (i != urls.length - 1) {
                  message.text = message.text.replace("：</a>", "，</a>");
                  message.text = message.text.replace(":</a>", "，</a>");
                } else {
                  message.text = message.text.replace("：</a>", "；</a>");
                  message.text = message.text.replace(":</a>", "；</a>");
          ...
          if (
            message.text.indexOf("https:") > -1 ||
            (message.text.indexOf("http:") > -1 && !isHistory)
          ) {
            // 常见问题中包含URL，超链接显示
            var reg =
              /(https?|http|ftp|file):\/\/[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]/g;
      ...
      generateMessageSection: function (message) {
        ...
        if (message["receiveId"] == kefu.chat.otherUser.id) {
          ...
          section.innerHTML =
            '<div class="head"></div><div class="sanjiao"></div><div class="text">' +
            kefu.ubb(message["text"]) +
            "</div>";
          ...
          section.innerHTML =
            ...
            kefu.getImageUrl(kefu.chat.otherUser.head) +
            ');"></div><div class="sanjiao"></div><div class="text">' +
            message["text"] +
            "</div>";
        }
        return section;
      ...
      generateSystemMessageSection: function (text) {
        var section = kefu.document.get().createElement("section");
        section.className = "chat otherUser MSG";
        section.innerHTML =
          '<div class="head" style="background-image: url(' +
          kefu.getImageUrl(kefu.chat.otherUser.head) +
          ');"></div><div class="sanjiao"></div><div class="text">' +
      ...
      scrollToBottom: function () {
        ...
        kefu.document
          ...
          .scrollTo(
            0,
            kefu.document.get().getElementById("chatcontent").scrollHeight
          );
      },
      ...
      appendSystemMessage: function (text) {
        ...
        chatcontent.innerHTML =
          chatcontent.innerHTML +
          '<section class="chat otherUser MSG">' +
          '<div class="head" style="background-image: url(' +
          kefu.getImageUrl(kefu.chat.otherUser.head) +
          ');"></div><div class="sanjiao"></div><div class="text">' +
          text +
      ...
      newMessageRemind: function (message) {
        kefu.cache.getUser(message.sendId, function (user) {
          var remindTextDiv = kefu.document
            .get()
            .getElementById("newMessageRemindText");
          ...
          //显示这条消息
          kefu.document.get().getElementById("newMessageRemind").style.display =
            "block";
        });
      },
      // 模糊查询
        ...
        $.ajax({
          ...
          success: (res) => {
            if (res.code == 0) {
              let data = res.data;
              countryList = data;
              kefu.ui.chat.renderCountry(data);
            }
          },
        });
      },
      ...
      renderCountry: function (data) {
        let html = "";
        data.map((v, index) => {
          html += `<li onclick="kefu.ui.chat.chooseCountry('${v.country_name_en}')">
            ${v.country_name_en}
          </li>`;
        });
        $("#country-chat").html(html);
      },

      // 获取点击选择的国家
      chooseCountry: function (value) {
        $("#country-chat-input").val(value);
        kefu.ui.chat.closeCountryUl();
      },
      // 获取点击选择的business
      chooseBusiness: function (value) {
        $("#business-chat-input").val(value);
        kefu.ui.chat.closeBusinessUl();
      },
      // 监听国家输入
      changeCountry: function () {
        let value = $("#country-chat-input").val();
        if (kefu.ui.chat.fuzzyQuery(countryList, value).length > 0) {
          kefu.ui.chat.renderCountry(
            kefu.ui.chat.fuzzyQuery(countryList, value)
          );
        }
        if (!value) {
          kefu.ui.chat.getCountryList();
        }
      },
      // 打开联系我们
      ...
      calAge: function (e) {
        if (e.ctrlKey && e.keyCode == 13) {
          kefu.document.get().getElementById("text").value += "\n";
          return;
        }
        if (e.keyCode == 13) {
          kefu.chat.sendButtonClick();
          setTimeout(() => {
            kefu.document.get().getElementById("text").value = "";
          }, 10);
        }
      ...
      render: function (otherUserId) {
        kefu.ui.chat.html = kefu.ui.chat.html.replace(
          /{shuruTypeColor}/g,
          kefu.ui.color.shuruTypeColor
        ); //颜色替换，发送按钮颜色遵循 kefu.ui.color.shuruTypeColor 的设置。 v1.2 增加
        if (kefu.ui.list.renderAreaId.length > 0) {
          //有设置渲染区域，那么渲染到设置的id上
          if (
            kefu.document.get().getElementById(kefu.ui.chat.renderAreaId) !=
            null
          ) {
            kefu.document
              .get()
              .getElementById(kefu.ui.chat.renderAreaId).innerHTML =
              kefu.ui.chat.html;
          }
        } else {
        ...
        kefu.chat.otherUser = {
          id: otherUserId,
          nickname: "加载中..",
          head: "/images/chat/head.png",
        };
        var chatCacheList = kefu.cache.getUserMessageList(
          kefu.chat.otherUser.id
        );
        ...
        kefu.chat.getOtherUser(otherUserId, function (data) {
          ...
          //设置网页聊天窗title 的对方昵称
          kefu.document.get().getElementById("nickname").innerHTML =
            kefu.chat.otherUser.nickname;
          //对方在线状态
          //   kefu.document.get().getElementById("onlineState").innerHTML =
          //     data.onlineState;
          if (kefu.document.get().getElementById("otherUserHead") != null) {
            //聊天窗口中对方用户的头像
            kefu.document.get().getElementById("otherUserHead").src =
              kefu.getImageUrl(data.user.head);
          }
          ...
          try {
            ...
            for (var i = 0; i < heads.length; i++) {
              heads[i].getElementsByClassName("head")[0].style.backgroundImage =
                "url('" + kefu.getImageUrl(kefu.chat.otherUser.head) + "')";
            }
          } catch (e) {
            console.log(
              "当前chat聊天模板中没有显示头像吧？下面这个错误只是个提示，无需理会"
            );
            console.log(e);
          }
          ...
          var autoReplyInterval = setInterval(function () {
            if (
              typeof kefu.chat.otherUser.id != "undefined" &&
              kefu.user != null &&
              typeof kefu.user.id != "undefined"
            ) {
              if (
                typeof kefu.socket.socket != "undefined" &&
                kefu.socket.socket != null &&
                kefu.socket.socket.readyState == kefu.socket.socket.OPEN
              ) {
                //socket也已经打开了
                //拉对方的自动回复欢迎语
                kefu.socket.send(
          ...
          kefu.document.get().getElementById("chatcontent").onscroll =
            function () {
              //console.log(document.getElementById('chatcontent').scrollTop);
              if (
                kefu.document.get().getElementById("chatcontent").scrollTop <
                900
              ) {
                //还剩一页半了，那么提前开始加载上一页
      ...
      pc: {
        ...
        close: function () {
          kefu.document
            .get()
            .getElementById("pc")
            .parentNode.removeChild(kefu.document.get().getElementById("pc"));
          kefu.ui.list.entry();
        },
        ...
        init: function () {
          ...
          kefu.extend.pc = {
            initChat: function () {
              ...
              kefu.document.get().getElementById("close").innerHTML =
                kefu.ui.images.close.replace(
                  /{color}/g,
                  kefu.ui.color.extendIconColor
                );
            },
          };
        ...
        sizeChange: {
          ...
          moveInit: function () {
            ...
            var contentWindows = kefu.document
              .get()
              .getElementById("chatcontent");
            ...
            console.log(
              "chatWindowsHeight:" +
                chatWindowsHeight +
                ", contentWindowsHeight:" +
                contentWindowsHeight
            );
            kefu.ui.chat.pc.sizeChange.height_chat_content_size =
              chatWindowsHeight - contentWindowsHeight;
            //缩放效果
            ...
            scale.onmousedown = function (e) {
              ...
              father.onmousemove = function (ev) {
                ...
                w =
                  w >=
                  father.offsetWidth - kefu.ui.chat.pc.chatWindowDiv.offsetLeft
                    ? father.offsetWidth -
                      kefu.ui.chat.pc.chatWindowDiv.offsetLeft
                    : w;
                ...
                h =
                  father.offsetHeight - kefu.ui.chat.pc.chatWindowDiv.offsetTop;
                //console.log("w:"+w+", h:"+h);

                if (kefu.ui.chat.pc.sizeChange.currentY > 0) {
                  var height =
                    kefu.ui.chat.pc.sizeChange.chatWindowHeight -
                    (kefu.ui.chat.pc.sizeChange.currentY - ev.clientY);
                  console.log(
                    kefu.ui.chat.pc.sizeChange.currentY -
                      ev.clientY +
                      ", height:" +
                      height
                  );
                  ...
                  kefu.document
                    .get()
                    .getElementById("chatcontent").style.height =
                    height -
                    kefu.ui.chat.pc.sizeChange.height_chat_content_size +
                    "px";
                } else {
                  kefu.ui.chat.pc.sizeChange.chatWindowHeight =
                    kefu.ui.chat.pc.chatWindowDiv.offsetHeight;
                }
        ...
        //鼠标点击移动、放大缩小的初始化
        moveInit: function () {
          kefu.ui.chat.pc.chatWindowDiv = kefu.document
            .get()
            .getElementById("pc");
          kefu.ui.chat.pc.chatHeadWindowDiv = kefu.document
            .get()
            .getElementById("head");
  ...
  chat: {
    ...
    getOtherUser: function (chatid, func) {
      ...
      if (chatid.indexOf("kefuchatid_") == 0) {
        //是kefu.chatid
        if (
          kefu.api.getChatOtherUserByKefuChatid == null ||
          kefu.api.getChatOtherUserByKefuChatid.length < 1
        ) {
          msg.popups(
            "请设置 kefu.api.getUserByKefuChatId 接口，用于获取分配的客服坐席"
          );
          return;
        }
        ...
        //是正常的chatid
        if (
          kefu.api.getChatOtherUser == null ||
          kefu.api.getChatOtherUser.length < 1
        ) {
          msg.popups(
            "请设置 kefu.api.getChatOtherUser 接口，用于获取跟我沟通的对方的信息"
          );
          return;
        }
      ...
      request.post(
        gainApiUrl,
        { token: kefu.token.get(), id: chatid },
        function (data) {
    ...
    loadHistoryList() {
      if (!kefu.chat.currentLoadHistoryList) {
        ...
        if (
          kefu.cache.getUserMessageList(kefu.chat.otherUser.id).length <
          kefu.cache.everyUserNumber
        ) {
        ...
        section.innerHTML =
          '<div class="text systemText">历史聊天加载中...</div>';
        chatcontent.insertBefore(section, firstItem);

        //创建网络请求
        request.post(
          kefu.api.get(kefu.api.chatLog),
          {
            token: kefu.token.get(),
            otherId: kefu.chat.otherUser.id,
            time: kefu.chat.chatMessageStartTime,
            type: "before",
          },
          function (data) {
            ...
            var chatcontent = kefu.document.get().getElementById("chatcontent");
            //删除聊天记录加载中的提示
            chatcontent.removeChild(
              kefu.document.get().getElementById("historyListLoading")
            );
            ...
            if (data.result == "0") {
              ...
              if (data.number > 0) {
                ...
                chatcontent.insertBefore(
                  kefu.ui.chat.generateSystemMessageSection("没有更多了"),
                  firstItem
                );
              }
            }
    ...
    sendPluginMessage: function (data, name) {
      if (name == null) {
        msg.popups(
          "kefu.chat.sendPluginMessage(data,name) 方法中，请传入name的值。<br/>name是发送这个消息的插件的名字，比如这个插件是 kefu.extend.explain ，那么这里传入的是 'explain'"
        );
        return;
      }
      if (data != null) {
      ...
      //更新聊天窗口
      if (
        kefu.extend[name] != null &&
        typeof kefu.extend[name] == "undefined" &&
        typeof kefu.extend[name].format != "undefined"
      ) {
        ...
        console.log(
          "提示: " +
            name +
            "插件中， format 方法未实现，那么聊天消息体中这个插件的消息将不会出现。如果你想这个插件发送的消息能出现在聊天窗口中，请重写实现这个插件的 format 方法。可参考文档：  https://gitee.com/leimingyun/dashboard/wikis/leimingyun/kefujs-api-interface-of/preview?sort_id=3663420&doc_id=1274007"
        );
      }
    ...
    sendButtonClick: function () {
      var value = kefu.document.get().getElementById("text").value;
      if (value.length == 0) {
        //msg.info("尚未输入");
        return;
      }
    ...
    shuruTypeChange: function () {
      ...
      if (kefu.chat.shuruType == "jianpan") {
        //当前是键盘输入，切换到更多输入方式
        kefu.chat.shuruType = "more";
        kefu.document.get().getElementById("shuruType").value =
          kefu.ui.images.jianpan.replace(
            /{color}/g,
            kefu.ui.color.shuruTypeColor
          );
        ...
        for (var key in kefu.extend) {
          if (
            kefu.extend[key].icon != null &&
            kefu.extend[key].icon.length > 0
          ) {
            inputExtendHtml =
              inputExtendHtml +
              '<div class="item" onclick="kefu.extend[\'' +
              key +
              '\'].onclick();"><div class="iconButton">' +
              kefu.extend[key].icon.replace(
                /{color}/g,
                kefu.ui.color.extendIconColor
              ) +
              '</div><div class="iconName">' +
              kefu.extend[key].name +
              "</div></div>";
          }
        }
        kefu.document.get().getElementById("inputExtend").innerHTML =
          inputExtendHtml;
        //显示扩展功能栏
        kefu.document.get().getElementById("inputExtend").style.display = "";
        ...
        kefu.document
          .get()
          .getElementById("inputExtendShowArea").style.display = "";
      } else {
        //当前是更多输入，切换到键盘输入方式
        kefu.chat.shuruType = "jianpan";
        kefu.document.get().getElementById("shuruType").innerHTML =
          kefu.ui.images.more.replace(/{color}/g, kefu.ui.color.shuruTypeColor);
        ...
        for (var key in kefu.extend) {
          if (
            ...
            inputExtendHtml =
              inputExtendHtml +
              '<span class="smallIcon" onclick="kefu.extend[\'' +
              key +
              "'].onclick();\">" +
              kefu.extend[key].icon.replace(
                /{color}/g,
        ...
        kefu.document.get().getElementById("inputExtend").innerHTML =
          '<div class="extendSmallIcon">' + inputExtendHtml + "</div>";
      }
    },
    //切换到键盘输入类型
  ...
  cache: {
    ...
    add: function (message) {
      ...
      if (otherUserId != "0" && otherUserId.length > 0) {
        ...
        if (
          kefu.chat.otherUser != null &&
          kefu.chat.otherUser.id != null &&
          kefu.chat.otherUser.id == otherUserId
        ) {
    ...
    getUser: function (userid, func) {
      ...
      if (userStr == null || userStr.length < 1) {
        //从网络获取
        request.send(
          kefu.api.get(kefu.api.getChatOtherUser),
          { token: kefu.token.get(), id: userid },
          function (data) {
            //请求完成
            if (data.result == "1") {
              user = data.user;
  ...
  extend: {
    ...
    face: {
      ...
      onclick: function () {
        ...
        for (var key in kefu.extend.face.faces) {
          html =
            html +
            "<span onclick=\"kefu.extend.face.insert('" +
            key +
            "');\">" +
            kefu.extend.face.faces[key] +
            "</span>";
        }
        html = html + "</div>";
    ...
    image: {
      ...
      template:
        '<img style="max-width: 100%;" onclick="kefu.extend.image.fullScreen(\'{url}\');" src="{url}" />',
      initChat: function () {
        var inputEle = kefu.document.get().createElement("input");
        inputEle.setAttribute(
          "accept",
          "image/gif,image/jpeg,image/jpg,image/png,image/svg,image/bmp"
        );
      ...
      /* 将message.extend 的json消息格式化为对话框中正常浏览的消息 */
      format: function (message) {
        message.text = kefu.extend.image.template.replace(
          /{url}/g,
          kefu.filterXSS(kefu.getImageUrl(message.extend.url))
        );
        return message;
      },
      onclick: function () {
        ...
        if (kefu.document.get().getElementById("imageInput").oninput == null) {
          kefu.document.get().getElementById("imageInput").oninput = function (
            e
          ) {
            if (typeof e.srcElement.files[0] != "undefined") {
              ...
              msg.loading("上传中");
              request.upload(
                kefu.api.get(kefu.api.uploadImage),
                { token: kefu.token.get() },
                file,
                function (data) {
                  msg.close();
    ...
    luyin: {
      ...
      onclick: function () {
        if (window.location.protocol == "http:") {
          msg.info(
            "发送语音功能无法使用。<br/>因为当前网页的协议是http协议，只有在https协议下才能使用此功能。"
          );
          return;
        }
        try {
      ...
      format: function (message) {
        message.text =
          '<audio src="' +
          kefu.filterXSS(message.extend.url) +
          '" controls="controls" style="max-width: 100%;">您的浏览器不支持 audio 标签。</audio>';
        return message;
      },
      recorder: null,
      //开始录音
      startLuyin: function (obj) {
        ...
        //弹出黑窗的关闭按钮，点击关闭将不在录音
        if (
          kefu.document.get().getElementsByClassName("msg_close").length > 0
        ) {
          kefu.document.get().getElementsByClassName("msg_close")[0].onclick =
            function () {
    ...
    file: {
      ...
      format: function (message) {
        message.text =
          ...
          message.extend.url +
          '\');"><div style="width: 3rem; float: left; height: 3rem;">' +
          kefu.extend.file.icon.replace(
            /{color}/g,
            kefu.ui.color.extendIconColor
          ) +
          '</div><div style="text-align: left; font-size: 0.9rem; line-height: 1.4rem;width: 100%;margin-left: 3.6rem;"><div style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 70%; font-size: 1rem;">' +
          kefu.filterXSS(message.extend.fileName) +
          '</div><div style="font-size: 0.7rem;">大小:' +
          message.extend.size / 1 +
          "KB</div></div></div>";
        return message;
      },
    },
    ...
    order: {
      ...
      onclick: function () {
        msg.loading("获取中");
        request.post(
          kefu.extend.order.requestApi,
          {
            token: kefu.token.get(),
            zuoxiid: kefu.chat.otherUser.id,
            myid: kefu.user.id,
          },
          function (data) {
            msg.close();
      ...
      otherShow: function (orderid) {
        if (
          ...
          if (
            typeof window.webkit.messageHandlers.appShowOrder.postMessage ==
            "function"
          ) {
    ...
    goods: {
      ...
      otherShow: function (goodsid) {
        if (
          ...
          //ios上用
          if (
            typeof window.webkit.messageHandlers.appShowGoods.postMessage ==
            "function"
          ) {
  ...
  socket: {
    ...
    heartBeat: {
      ...
      startHeartBeat: function () {
        if (kefu.socket.heartBeat.isStart == false) {
          ...
          console.log(
            "kefu.socket headrtBeat thread start , time: " +
              kefu.socket.heartBeat.time +
              "s"
          );
        }
      },
    ...
    onmessage: function (res) {
      ...
      if (kefu.mode == "pc") {
        ...
        if (kefu.currentPage == "list") {
          ...
          //当前在chat,如果当前的chat沟通对象跟消息都是一个人，那么显示在当前chat
          if (message.text.indexOf("请留资") > -1) {
            console.log(message);
            let str = message.text;
            message.text = str.replace(
              "请留资",
              '<div onclick="kefu.ui.chat.openContactUs()" style="color:#9ac2b8;cursor: pointer;text-decoration: underline;" id =“info_btn”>Contact Us</div>'
            );
            // message.text = "您好，非常感谢您支持捷途品牌，为了给您更好的解答您的问题，请您留下联络方式，我们将尽快与您取得联系。<br><div id =“info_btn”>留资表单</div>"
            // document.getElementById("dialog").style.display = "block";
            // document.getElementById("head").style.display = "none";
          }
          if (
            message.sendId == kefu.chat.otherUser.id ||
            message.type == "SYSTEM"
          ) {
            message.read = true;
            kefu.ui.chat.appendMessage(message); //聊天窗口增加消息
    ...
    connect: function (url) {
      ...
      var socketCloseAgainConnectInterval = setInterval(function () {
        if (
          typeof kefu.socket.socket != "undefined" &&
          kefu.socket.socket != null &&
          kefu.socket.socket.readyState == kefu.socket.socket.CLOSED
        ) {
          console.log(
            "socketCloseAgainConnectInterval : socket closed , again connect ..."
          );
    ...
    send: function (text) {
      if (
        ...
        kefu.socket.socket == null ||
        kefu.socket.socket.readyState == kefu.socket.socket.CLOSED ||
        kefu.socket.socket.readyState == kefu.socket.socket.CLOSING
      ) {
